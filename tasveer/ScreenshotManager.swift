...
class ScreenshotManager: ObservableObject {
    // Development mode: bypass permissions and always use a sample image
    ...
    private func ensureAvailableContent() async -> SCShareableContent? {
        ...
        do {
            ...
            if errorMessage.contains("not permitted") || errorMessage.contains("authorization") || errorMessage.contains("permission") {
                    ...
                    await MainActor.run {
                        ...
                        if !granted {
                            self.presentError(
                                ...
                                message: "Tasveer needs screen recording permission to capture screenshots.\n\n1. Open System Settings\n2. Go to Privacy & Security > Screen Recording\n3. Enable Tasveer\n4. Restart the app if needed"
                            )
                        }
                    }
                } else {
                    // We have permission but still failed - try once more
                    do {
                        let content = try await SCShareableContent.current
                        self.availableContent = content
                        return content
                        ...
                        await MainActor.run {
                            self.presentError(
                                "Screenshot Error",
                                message: "Failed to access screen content despite having permission: \(error.localizedDescription)"
                            )
                        }
                    }
                }
                ...
                await MainActor.run {
                    self.presentError(
                        "Screenshot Error",
                        message: "Failed to capture screen content: \(error.localizedDescription)"
                    )
                }
            }
            return nil
    ...
    func captureArea(delay: TimeInterval = 0) {
        // Dev mode: always use sample image
        if isDevelopmentMode {
...
extension ScreenshotManager {
    ...
    fileprivate func makeSampleCGImage(size: CGSize = CGSize(width: 1280, height: 800)) -> CGImage? {
        ...
        let sampleURL = URL(fileURLWithPath: "/Users/superman41/Downloads/pexels-yankrukov-8837370.jpg")
